...
library(knitr)
library(jsonlite)   # reads the BC boundary for the map in Figure 6
...
Generalized national allometric equations give forest-carbon accounting consistent, transferable biomass estimates where local destructive equations are missing, at a known cost in accuracy for taxa and components under-represented in the calibration. Stem bark is such a component in the larches, whose two North American species differ sharply in bark investment. In a first stage we evaluated the stem-bark component of Canada's national biomass coefficients against destructively measured bark mass for tamarack (*Larix laricina*, from the ENFOR programme, the calibration data, n = 439) and western larch (*Larix occidentalis*, from the LegacyTreeData repository, independent, n = 13), and fitted larch-specific equations. Western larch has no national coefficient, so its bark is taken from the generic conifer coefficient, which underestimated western-larch stem bark by 47 percent; a fitted western-larch equation reduced cross-validated relative error from 56 to 9 percent, with an R squared of 0.99. The same generic coefficient over-predicted tamarack bark by 19 percent and increasingly with size, while tamarack's own coefficient was near unbiased. Western-larch stem bark is 18.9 percent of stem mass against 11.5 percent for tamarack, so the two larches depart in opposite directions and a species-specific western-larch coefficient is warranted whereas a genus-pooled one is not. Independent stem-taper measurements on trees sharing none of the bark-mass individuals and reaching 39.4 cm confirmed the divergence, bark occupying 24.4 percent of the stem cross-section in western larch against 13.3 percent in tamarack, a ratio held within their overlapping diameter range, and showed that bark cross-sectional area scales with diameter to the power 2.56 in western larch against 1.36 in tamarack, so bark's share of the stem rises with size in the one species and falls in the other. In a second stage we tested the equations province-wide on the British Columbia Vegetation Resources Inventory, applying the national and locally fitted equations to 67,409 larch-leading polygons. Correcting the western-larch bias raised British Columbia's western-larch stem-bark carbon from 3.15 to 5.03 teragrams, an increase of 1.9 teragrams or 60 percent over 376,000 hectares, concentrated in the southeast interior, while tamarack fell 12 percent. Because our western-larch equation is calibrated below 20 cm, we also applied an independently published western-larch equation fitted on trees to 105 cm, which raised the same pool to 4.31 teragrams, so the correction across British Columbia is bracketed at 37 to 60 percent, 1.2 to 1.9 teragrams. That equation exceeds ours above 45 cm, placing our extrapolation on the conservative side in the large trees that hold most stand carbon. The results locate where the national stem-bark coefficients serve the larches and where a targeted, species-specific refinement changes British Columbia's carbon account.
...
# provincial outline for orientation: Natural Earth 1:50m admin-1, public domain,
# dissolved to British Columbia and reprojected to BC Albers (EPSG:3005)
bc <- jsonlite::fromJSON(file.path(repo, "02.inputs/vri/bc_boundary_3005.geojson"),
                         simplifyVector = FALSE)
rings <- unlist(lapply(bc$features[[1]]$geometry$coordinates, function(poly)
  lapply(poly, function(r) do.call(rbind, lapply(r, unlist)))), recursive = FALSE)
xr <- range(vapply(rings, function(m) range(m[, 1]), numeric(2)))
yr <- range(vapply(rings, function(m) range(m[, 2]), numeric(2)))
...
plot(NA, xlim = xr, ylim = yr, asp = 1, cex.main = .85,
     xlab = "Easting (m, BC Albers)", ylab = "Northing (m)",
     main = "Larch stem-bark carbon correction across BC (local - national, Mg C/ha)")
for (m in rings) polygon(m[, 1], m[, 2], col = "grey96", border = "grey55", lwd = .6)
points(clean$LABEL_CENTRE_X, clean$LABEL_CENTRE_Y, pch = 16, cex = .18,
       col = adjustcolor(pal[cut(z, br, include.lowest = TRUE)], .6))
for (m in rings) polygon(m[, 1], m[, 2], border = "grey35", lwd = .7)
